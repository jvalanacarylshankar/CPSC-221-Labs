...
void update(Node*& root, Node *target, int key)
{
   if(root == NULL)
     return;
   ...
   update(root->left, target, key);
   update(root->right, target, key);
}
...
bool delete_node(Node*& root, int key) {
  ...
  // case 3: target has two children
       ...
       Node *min = find_min(target->left);
       delete_node(root, min->key);
       update(root, target, min->key);
       delete min;
       return true;
   }
...
int height( Node* x ) {
  ...
  else if (x->left == NULL and x->right == NULL){		return 0;}
  else{
 ...
 */
int depth( Node* root, Node* x, int level = 0) {
  if (root == NULL)
        return 0;
